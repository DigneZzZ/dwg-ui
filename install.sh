...
printf "${BLUE} Сейчас проверим свободен ли порт 51821 и не установлен ли другой wireguard.${NC}"

if [[ $(docker ps -q --filter "name=wireguard") ]]; then
    echo "!!!!>>> Другой Wireguard контейнер уже запущен, и вероятно занимает порт 51821. Пожалуйста удалите его и запустите скрипт заново "
    printf "${RED} !!!!>>> Завершаю скрипт! ${NC}"
    exit 1
else
    echo "Wireguard контейнер не запущен в докер. Можно продолжать"
    # Проверка, запущен ли контейнер, использующий порт 51821
    if docker ps -q --filter "publish=51821" | grep -q . ; then
    echo ""
    printf "${RED}!!!!>>> Порт 51821 уже используется контейнером.! ${NC}"
    printf "${RED} ******* Завершаю скрипт! ${NC}"
    exit 1
else
    echo "Порт 51821 свободен."
fi
printf "${GREEN} Проверили и убедились что другого контейнера использующего 51821 порт нет, в т.ч. контейнера Wireguard .. а значит можно продолжить установку${NC}"
fi
printf "${GREEN} Этап проверки докера закончен, можно продолжить установку${NC}"
